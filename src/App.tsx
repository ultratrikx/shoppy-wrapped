...
export function App() {
  ...
  useEffect(() => {
    if (orders !== undefined && currentUser !== undefined && savedProducts !== undefined) {
      analyzeShoppingData()
    }
  }, [orders, currentUser, savedProducts])
  ...
  const analyzeShoppingData = async () => {
    ...
    const totalProducts = orders.reduce((acc, order) => acc + (order.lineItems?.length || 0), 0)
    const uniqueShops = new Set(orders.map(order => order.shop?.name)).size
    
    const totalSpent = orders.reduce((acc, order) => {
      // Use estimated total since exact totalPrice might not be available
      const lineItemsTotal = order.lineItems?.reduce((itemAcc, item) => itemAcc + (item.quantity || 1), 0) || 0
    ...
    const favoriteShop = Object.entries(shopCounts).sort(([,a], [,b]) => b - a)[0]?.[0]
    ...
    const orderDates = orders.map(order => new Date()).sort((a, b) => b.getTime() - a.getTime())
    let streak = 0
  ...
  const prevFrame = () => {
    const prevIndex = currentFrameIndex === 0 ? storyFrames.length - 1 : currentFrameIndex - 1
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        if (navigator.share && navigator.canShare) {
          const file = new File([blob], 'shopping-wrapped-2024.png', { 
            type: 'image/png',
        ...
        if (isMobile) {
          ...
          try {
            ...
            link.click()
            alert('Image downloaded! Share it to your Instagram Story! 📱')
          }
        } else {
  ...
  const renderFrame = () => {
    switch (currentFrame) {
      ...
      case 'personality':
        return <PersonalityFrame stats={stats} />
          ...
          {storyFrames.map((_, index) => (
            <div
              ...
              className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                index <= currentFrameIndex ? 'bg-white' : 'bg-white/30'
              }`}
            />
          ))}
        </div>
...
const WelcomeFrame = ({ onStart }: { onStart: () => void }) => (
  <div className="h-full bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 flex flex-col items-center justify-center text-white p-8">
    <div className="text-center space-y-8">
      <div className="text-6xl mb-4">🛍️</div>
      <h1 className="text-4xl font-bold">Shopping<br />Wrapped</h1>
      <p className="text-lg opacity-80">Your year in shopping,<br />beautifully visualized</p>
      <button 
        onClick={onStart}
    ...
    <div className="absolute bottom-8 text-sm opacity-60">Tap to navigate →</div>
  </div>
)

// Analyzing Frame
const AnalyzingFrame = ({ progress }: { progress: number }) => (
  <div className="h-full bg-gradient-to-br from-purple-600 via-purple-700 to-purple-800 flex flex-col items-center justify-center text-white p-8">
    <div className="text-center space-y-8">
      ...
      <h2 className="text-3xl font-bold">Analyzing your<br />shopping journey...</h2>
...
const TotalSpentFrame = ({ amount }: { amount: number }) => (
  <div className="h-full bg-gradient-to-br from-green-500 via-green-600 to-green-700 flex flex-col items-center justify-center text-white p-8 relative overflow-hidden">
    ...
    <div className="absolute top-32 right-12 text-3xl animate-bounce" style={{animationDelay: '0.5s'}}>💳</div>
    <div className="absolute bottom-32 left-12 text-4xl animate-bounce" style={{animationDelay: '1s'}}>🛒</div>
    <div className="absolute bottom-20 right-8 text-3xl animate-bounce" style={{animationDelay: '1.5s'}}>💎</div>
  ...
  <div className="h-full bg-gradient-to-br from-orange-500 via-orange-600 to-orange-700 flex flex-col items-center justify-center text-white p-8">
    <div className="text-center space-y-8">
      ...
      <div className="text-lg opacity-70">
        {count > 50 ? "🛍️ Shopping champion!" : 
         count > 20 ? "🎯 Regular shopper" : 
...
const FavoriteShopFrame = ({ shop }: { shop?: string }) => (
  <div className="h-full bg-gradient-to-br from-pink-500 via-pink-600 to-pink-700 flex flex-col items-center justify-center text-white p-8">
    <div className="text-center space-y-8">
...
const TopProductFrame = ({ product }: { product?: string }) => (
  <div className="h-full bg-gradient-to-br from-indigo-500 via-indigo-600 to-indigo-700 flex flex-col items-center justify-center text-white p-8">
    <div className="text-center space-y-8">
      <div className="text-6xl mb-4">⭐</div>
      <h2 className="text-2xl font-light opacity-90">Most ordered item</h2>
...
const ShoppingStyleFrame = ({ stats }: { stats: ShoppingStats }) => (
  <div className="h-full bg-gradient-to-br from-teal-500 via-teal-600 to-teal-700 flex flex-col items-center justify-center text-white p-8">
    <div className="text-center space-y-8">
      <div className="text-6xl mb-4">🎨</div>
      <h2 className="text-2xl font-light opacity-90">Your shopping style</h2>
      <div className="text-3xl font-bold">
        {stats.avgOrderValue > 100 ? "Premium Collector" :
         stats.totalOrders > 30 ? "Frequent Explorer" :
         stats.savedProductsCount > 20 ? "Wishlist Curator" :
         "Mindful Shopper"}
      </div>
...
const YearInNumbersFrame = ({ stats }: { stats: ShoppingStats }) => (
  <div className="h-full bg-gradient-to-br from-red-500 via-red-600 to-red-700 flex flex-col items-center justify-center text-white p-8">
    <div className="text-center space-y-6">
      ...
      <div className="grid grid-cols-2 gap-6 max-w-sm">
        ...
        <div className="text-center">
          <div className="text-4xl font-bold">{stats.savedProductsCount}</div>
          <div className="text-sm opacity-80">items saved</div>
        </div>
        <div className="text-center">
          <div className="text-4xl font-bold">{stats.uniqueShops}</div>
          <div className="text-sm opacity-80">shops visited</div>
        </div>
        <div className="text-center">
          <div className="text-4xl font-bold">{stats.shoppingStreak}</div>
...
// Personality Frame
const PersonalityFrame = ({ stats }: { stats: ShoppingStats }) => (
  <div className="h-full bg-gradient-to-br from-purple-500 via-purple-600 to-purple-700 flex flex-col items-center justify-center text-white p-8">
    <div className="text-center space-y-8">
      <div className="text-6xl mb-4">✨</div>
      <h2 className="text-2xl font-light opacity-90">You are a</h2>
      <div className="text-3xl font-bold italic">
        {stats.totalSpent > 1000 && stats.avgOrderValue > 100 ? "Luxury Enthusiast" :
         stats.totalOrders > 40 ? "Shopping Adventurer" :
         stats.savedProductsCount > stats.totalProducts ? "Thoughtful Planner" :
         stats.uniqueShops > 10 ? "Brand Explorer" :
         "Conscious Consumer"}
      </div>
      <p className="text-lg opacity-80 max-w-xs">
        {stats.totalSpent > 1000 ? "You appreciate quality and aren't afraid to invest" :
         stats.totalOrders > 40 ? "Always discovering new products and experiences" :
         "You take time to make thoughtful shopping decisions"}
      </p>
    </div>
  </div>
)
...
const ShareFrame = ({ 
  ...
  <div className="h-full bg-gradient-to-br from-pink-600 via-purple-600 to-indigo-600 flex flex-col items-center justify-center text-white p-8">
    <div className="text-center space-y-8">
      <div className="text-6xl mb-4">🎉</div>
      <h2 className="text-3xl font-bold">Shopping Wrapped<br />2024</h2>
      <div className="space-y-4 opacity-90">
        ...
        {stats.favoriteShop && (
          <p className="text-lg opacity-80">Favorite: {stats.favoriteShop}</p>
        )}
      </div>
